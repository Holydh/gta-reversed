...
int __stdcall VRmodule::VRmoduleLoop() {
	if (!openxr_init("GTA SA VR", d3d_swapchain_fmt)) {
		d3d_shutdown();
...
bool openxr_init(const char *app_name, int64_t swapchain_format) {
    ...
    //XrResult createInstanceResult = xrCreateInstance(&createInfo, &xr_instance);
    //cout << createInstanceResult;
	if (xrCreateInstance(&createInfo, &xr_instance) != XR_SUCCESS)
		return false;
	...
	if (xrCreateSession(xr_instance, &sessionInfo, &xr_session) != XR_SUCCESS)
		return false;
...
void openxr_shutdown() {
	...
	for (int32_t i = 0; i < (int32_t)xr_swapchains.size(); i++) {
